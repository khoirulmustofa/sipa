<?php
class M_validasi_ttd extends CI_Model{

  //upm
  function getDataValidasi($id_random){
    $hasil=$this->db->query("SELECT * FROM tbl_ttd_dospem WHERE id_random = '$id_random' AND status_validasi='Tervalidasi' AND status='Tersedia'");
    return $hasil;
  }

  function getDataValidasiPengantarFakultas($id_random){
    $hasil=$this->db->query("SELECT * FROM tbl_ttd_surat WHERE id_random = '$id_random' AND status_validasi='Tervalidasi' AND status='Tersedia'");
    return $hasil;
  }

  function getDataValidasiFakultas($id_random){
    $hasil=$this->db->query("SELECT * FROM tbl_ttd_surat WHERE id_random = '$id_random' AND status_validasi='Tervalidasi' AND status='Tersedia'");
    return $hasil;
  }
