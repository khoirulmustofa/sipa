...
class Rekapitulasi extends CI_Controller {
	...
	public function index()
	{
		...
		$x['combobox_ujian']=$this->m_rekapitulasi->combobox_ujian();	
		$this->load->view('public/part/header');
		$this->load->view('public/part/menu');
		$this->load->view('fakultas/v_rekapitulasi', $x);
	...
	public function cetak($data=null){
		if($data!=null){
	        ...
	        $pdf->ln(2);
	        $pdf->SetFont('Times','B',7);
	        $pdf->Cell(7,$TinggiSemua,'No',1,0,'C');
	        $pdf->Cell(35,$TinggiSemua,'Nama Pengawas',1,0,'C');
	        $pdf->Cell(35,$TinggiSemua,'Matakuliah',1,0,'C');
	        $pdf->Cell(10,$TinggiSemua,'Sem',1,0,'C');
	        $pdf->Cell(45,$TinggiSemua,'Prodi',1,0,'C');
	        $pdf->Cell(10,$TinggiSemua,'Kelas',1,0,'C');
			$pdf->Cell(70,$TinggiSemua,'Pengampu',1,0,'C');
	        $pdf->Cell(10,$TinggiSemua,'Jml Mhs',1,0,'C');
	        $pdf->Cell(10,$TinggiSemua,'Mhs/Mk',1,0,'C');
	        $pdf->Cell(10,$TinggiSemua,'Mhs/Dsn',1,0,'C');
	        $pdf->Cell(10,$TinggiSemua,'Mk/Dsn',1,0,'C');
	        $pdf->Cell(10,$TinggiSemua,'Kls/Dsn',1,1,'C');
	        ...
	        foreach($x->result_array() as $i):
				...
				$a = 0;
				foreach ($array_dosen as $npk_) {
					$nama_dosen_ = $this->m_rekapitulasi->getNamaDosen($npk_);
					$array_npk_hasil[$a] = $npk_;
					$array_dosen_hasil[$a] = $nama_dosen_;
					$a++;
				}
				$dosen_pengampu_str = implode('/ ', $array_dosen_hasil);
	            ...
	            $pdf->SetFont('Times','',6);
	            if($rowspan==false){
	                $pdf->Cell(7,$TinggiSemua,$no++,'L,T,R',0,'C');
	            }else{
	                $pdf->Cell(7,$TinggiSemua,'','R,L',0,'C');
	            }
	            ...
	            $pdf->Cell(45,$TinggiSemua,$prodi,1,0,'L');
	            $pdf->Cell(10,$TinggiSemua,$kelas,1,0,'C');
				$pdf->Cell(70,$TinggiSemua,substr($dosen_pengampu_str, 0, 60)."...",1,0,'L');
	            $pdf->Cell(10,$TinggiSemua,$jum_hadir,1,0,'C');
	            ...
	            if($rowspan2==false){
	                $pdf->Cell(10,$TinggiSemua,$count_mhs_per_mk,'L,T,R',0,'C');
	            ...
	            if($rowspan==false){
					$count_mhs = $this->m_rekapitulasi->j_mhs1($npk, $data);
	            	...
	            	$pdf->Cell(10,$TinggiSemua,$count_mhs,'L,T,R',0,'C');
	                $tot_mhs_hadir2 = $tot_mhs_hadir2 + $count_mhs;	 

	                $pdf->Cell(10,$TinggiSemua,$count_mk,'L,T,R',0,'C');
	                $pdf->Cell(10,$TinggiSemua,$count_kelas,'L,T,R',1,'C');
	                $tot_mk = $tot_mk + $count_mk;
	                $tot_kelas = $tot_kelas + $count_kelas;
	            }else{
	            	$pdf->Cell(10,$TinggiSemua,'','R,L',0,'C');
	                $pdf->Cell(10,$TinggiSemua,'','R,L',0,'C');
	                $pdf->Cell(10,$TinggiSemua,'','R,L',1,'C');
	            }
	        ...
	        $pdf->Cell(212,$TinggiSemua,'Total',1,0,'C');
	        $pdf->Cell(10,$TinggiSemua,$tot_mhs_hadir1,1,0,'C');
	        $pdf->Cell(10,$TinggiSemua,$tot_mhsMk,1,0,'C');
	        $pdf->Cell(10,$TinggiSemua,$tot_mhs_hadir2,1,0,'C');
	        $pdf->Cell(10,$TinggiSemua,$tot_mk,1,0,'C');
	        $pdf->Cell(10,$TinggiSemua,$tot_kelas,1,1,'C');
	        //END PENGAWAS 1  
